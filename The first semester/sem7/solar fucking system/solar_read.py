...
def parse_planet_parameters(line, planet):
    ...
    **planet** — объект планеты.
    ...
    assert (len(tokens) == 9)
    planet.R = int(tokens[2])
    planet.color = tokens[3]
    planet.m = float(tokens[4])
    planet.x = float(tokens[5])
    planet.y = float(tokens[6])
    planet.Vx = float(tokens[7])
    planet.Vy = float(tokens[8])
