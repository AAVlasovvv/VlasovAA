class Monoalphabet:
    # alphabet = 'эьормщднйгычясюцажшбтпвёлееъзхкфи'
    # keytable = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя'
    
    
    
    def __init__(self, keytable):
        self.alphabet = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя'
        lowercase_code = {x: y for x, y in zip(self.alphabet, keytable)}
        uppercase_code = {x.upper(): y.upper() for x, y in zip(self.alphabet, keytable)}
        
        delowercase_code = {x: y for x, y in zip(keytable, self.alphabet)}
        deuppercase_code = {x.upper(): y.upper() for x, y in zip(keytable, self.alphabet)}
        
        
        self._encode = lowercase_code
        self._encode.update(uppercase_code)
        self._decode = delowercase_code
        self._decode.update(deuppercase_code)
    
    def encode(self, text):
        return ''.join([self._encode.get(char, char) for char in text])
    
    def decode(self, text):
        return ''.join([self._decode.get(char, char) for char in text])


key = 'эьормщднйгычясюцажшбтпвёлеъузхкфи'
cipher = Monoalphabet(key)
line = input()
while line:
    print(cipher.decode(line))
    line = input()
    


